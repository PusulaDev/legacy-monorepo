...
import { cacheUtil, EnumCacheType, extendArray } from "@pusula/utils";
...
export class EnterpriseDataHouse implements IEnterpriseDataHouse {
    ...
    set<TModel>(type: EnumCacheType, typename: string, collection: TModel[], uniqueCacheKey?: string): void {
        ...
        cacheUtil.add(type, key, collection);
    }
    ...
    addItems<TModel>(
        ...
        } else cachedItems = this.get<TModel>(type, key);
        ...
        if (type == EnumCacheType.Memory) return;
