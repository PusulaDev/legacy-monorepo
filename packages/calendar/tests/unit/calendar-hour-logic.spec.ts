...
describe("Calendar Hour Logic", () => {
  ...
  it("should create minutes array with minuteInterval", () => {
    ...
    const start = "08:00";
    const end = "10:00";
    const shiftStart = "09:15";
    const minutesDefault = calendarHourLogic.createAllMinutes(start, end, minuteInterval);
    const minutesWithShiftStart = calendarHourLogic.createAllMinutes(start, end, minuteInterval, shiftStart);
    ...
    const expectedWithShiftStart: MinuteInterval[] = [
      { from: "08:00", to: "09:15", text: "00", hour: { text: "08", value: 8 }, interval: 75 },
      { from: "09:15", to: "09:40", text: "15", hour: { text: "09", value: 9 }, interval: 25 },
      { from: "09:40", to: "10:00", text: "40", hour: { text: "09", value: 9 }, interval: 20 },
    ];

    expect(minutesDefault).toEqual(expectedDefault);
    expect(minutesWithShiftStart).toEqual(expectedWithShiftStart);
  });
